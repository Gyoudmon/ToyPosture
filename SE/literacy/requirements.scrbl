...
@tamer-figure["ucg:research" "教研用例图"]{@image["stone/vsdx/ucg_research.png" #:scale 0.36]}
@tamer-figure["ucg:teach" "教学用例图"]{@image["stone/vsdx/ucg_teach.png" #:scale 0.36]}
...
接下来以部分用例来举例说明分析类在用例细化过程中的作用。
@tamer-figure["cg:store" "“材料登记库存”分析类图"]{@image["stone/vsdx/cg_store.png" #:scale 0.5]}
@tamer-figure["cg:check-stock" "“清点库存”用例细化"]{@image["stone/vsdx/cg_check_stock.png" #:scale 0.5]}

@handbook-action{用例举例: 材料登记入库}

“材料登记入库”用例与下面的“清点库存”用例差不多，都需要以可视化的形式展示存储空间和材料。
因此，我们首先关注这个过程中涉及的分析类的类图(@tamer-figure-ref{cg:store})。
...
都是实体类，可以持久化到数据库；它们同时也是边界类，负责将自己花在窗体上。
在详细设计阶段，它们这两个侧面会分开实现。

@handbook-action{用例举例: 清点库存}

“清点库存”用例与“材料登记入库”用例的主要差别在于，可能需要其他教师参与。
因此不在重复类图，而是以通信图(@tamer-figure-ref{cg:check-stock})
的形式展示其动态关系。
