...
传统GUI控件已经被别人画好了，开箱就用；Game UI则需要软件工程师自己动手来绘制所需要的可视对象。
本系统需要可视化房间平面图，传统GUI控件显然是不够用的。

采用该设计还有一个额外好处，可以借助动画系统构建测试脚本以自动化测试系统。
...
@itemlist[
 ...
 @item{特别地，IApparatuslet 表示需要将自己绘制在屏幕上的材料，需求分析阶段说过的分析类 Apparatus 的两个侧面，
  数据实体和图元，它们就在这里分化。IApparatuslet 的实现类各代表一种材料，
  其内部组合一个数据实体 Apparatus 的实例。}
