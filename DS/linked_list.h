 ...
 *   by supposing an instance of the `type` is located
 *   at position 0 in which case addresses of members
 *   are just the same as their offsets.
 */
#define offset_of(type, field) ((size_t)&((type*)0)->field)

/**
 * Get the pointer to the container instance of `type`
 *   based on the address of the member `field` inside the
 *   target instance simply by subtracting the offset of
 *   `field` in the `type`
 *
 * NOTE that pointer arithmetic is based on the size of the
 *   type of the pointer so that explicitly casting the
 *   instance pointer to `char *` is required before
