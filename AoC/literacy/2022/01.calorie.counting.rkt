...
检阅清单的过程就是个简单的@racketcommentfont{读取-记录-替换}循环（@racketidfont{rrsl}），
...
每一轮循环都相当于是一次（递归）函数调用，并且比对着那俩变量创建了新的位置，本该用来修改它们的值也被放到
新的位置去了，而变量原来的位置在哪，里面有什么都已经不重要了。这种思路需要专业引导，
初学者一时半会不能适应是正常现象。
...
还是看上面的例子，携带卡路里最多的三只精灵分别是第四只（@racket[24000] 卡）、
...
可以构成数组、列表、元组等复合类型。这些都可以，不过，为继续函数式风格，我这里选择用列表来实现。
在别的语言里，比如 Python，不可变的元组才更符合函数式风格。
...
相对比上面的 @racketid[find-maximum-calorie] 函数，这个复数形式的 @racketid[find-maximum-calories]
